...
| `test_hyperparams.py` | Adam: trajectory (learning rate, betas, bias correction) and epsilon. Momentum and epsilon of BatchNorm and LayerNorm, and dropout rates |
...
| `test_train_step.py` | One SGD step with learning rate 1, which exposes the gradient: same loss, same gradients and same BatchNorm statistics |
...
1. Add the combination to `MODELS` and its input shape to `INPUT_SHAPES`, in `helpers.py`.
2. If it brings a new layer type, add it to the `kinds` dictionaries in `transplant.py` and its
   ...
   gradient tests then cover it automatically.
